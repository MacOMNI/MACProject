   ...
   // [self.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    for (NSUInteger i=0; i<dataSource.count; i++) {
        ...
        if ([dataSource[i] isKindOfClass:[UIImage class]]) {
           ...
           [ iv yy_setImageWithURL:[NSURL URLWithString:dataSource[i]] placeholder:[UIImage imageNamed:@"placeholder_dropbox"]];
        }else if ([dataSource[i] isKindOfClass:[NSURL class]]){
            [ iv yy_setImageWithURL:dataSource[i] placeholder:[UIImage imageNamed:@"placeholder_dropbox"]];
        }
    }
